...
async function selectSpecies(name){
  ...
  try{selectedDex=await fetchDexPokemon(name);$('poke-species-search').value=selectedDex.species;renderSelectedSpecies();syncAbilitySelect();const blocked=usedInPriorRegion(selectedDex.slug,$('poke-region').value||state.currentRegion);$('species-note').textContent=blocked?`No disponible: ya fue utilizado en ${blocked}.`:'Ficha automática cargada.';}catch(e){selectedDex=null;$('species-note').textContent='No pude cargar esa especie. Comprueba la conexión o prueba otro nombre.';renderSelectedSpecies();syncAbilitySelect();}
