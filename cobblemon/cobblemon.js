...
function renderTeam(){
  ...
  mons.forEach(p=>{
    ...
    card.innerHTML=`<div class="poke-card-head"><div><div class="poke-name">${esc(p.nickname||p.species)}</div><div class="poke-species">${esc(p.species)}${p.level?` · Nv. ${esc(p.level)}`:''}</div></div><button class="more-btn" data-edit="${p.id}" title="Editar">•••</button></div><div class="poke-visual">${pokemonImg(p)}</div><div class="type-row compact">${typePills(p.types)}</div><div class="lives-row"><div class="lives clickable-lives">${hearts(p.lives,maxLives(p.region),p.id)}</div><span class="life-label ${Number(p.lives)===0?'out':''}">${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)}`}</span></div><div class="heart-hint">Haz clic en los corazones para ajustar las vidas</div>`;
...
function renderList(){
  ...
  mons.forEach(p=>{
    ...
    row.innerHTML=`<div class="list-mon"><div class="list-thumb">${pokemonImg(p)}</div><div class="row-title"><strong>${esc(p.nickname||p.species)}</strong><span>${esc(p.species)} · ${esc(p.region)}${p.level?` · Nv. ${esc(p.level)}`:''}</span><div class="type-row tiny">${typePills(p.types)}</div></div></div><div class="list-stats">${statsInline(p.stats)}</div><div class="list-lives clickable-lives">${hearts(p.lives,maxLives(p.region),p.id)}<small>${Number(p.lives)?'Activo':'Inutilizable'}</small></div><div class="list-flags">${p.inTeam?'<span class="badge live">Equipo</span>':''}${p.champion?'<span class="badge warn">🏆</span>':''}</div><div class="row-actions"><button class="btn small" data-team="${p.id}">${p.inTeam?'Quitar':'Equipo'}</button><button class="btn small" data-edit="${p.id}">Editar</button></div>`;
  ...
  list.querySelectorAll('[data-heart-id]').forEach(b=>b.onclick=()=>setLifeFromHeart(b.dataset.heartId,Number(b.dataset.heartValue)));list.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>openEdit(b.dataset.edit));
}
