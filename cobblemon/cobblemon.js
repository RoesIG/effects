...
function selectedExpandedStats(stats={}){
  const total=STAT_ORDER.reduce((sum,k)=>sum+(Number(stats?.[k])||0),0);
  const items=STAT_ORDER.map(k=>{
    const raw=Number(stats?.[k]);const value=Number.isFinite(raw)?raw:0;const pct=Math.max(0,Math.min(100,(value/255)*100));
    return `<div class="selected-stat-card"><div class="selected-stat-top"><span>${statLabel[k]}</span><strong>${value||'—'}</strong></div><div class="selected-stat-track"><span style="width:${value?Math.max(3,pct):0}%"></span></div></div>`;
  }).join('');
  return `<div class="selected-stats-expand"><div class="selected-stats-expand-head"><strong>STATS BASE</strong><span>Total ${total}</span></div><div class="selected-stats-grid">${items}</div></div>`;
}
...
function renderSelectedSpecies(){
  ...
  box.classList.toggle('has-data',!!selectedDex);
