...
function changeLife(id,delta){const p=state.pokemon.find(x=>x.id===id);if(!p)return;const before=Number(p.lives);p.lives=Math.max(0,Math.min(3,before+delta));if(delta<0&&p.lives<before){p.lifeHistory=Array.isArray(p.lifeHistory)?p.lifeHistory:[];p.lifeHistory.push({region:p.region||state.currentRegion,remaining:p.lives,at:new Date().toISOString()});}if(p.lives===0){p.inTeam=false;p.outRegion=p.outRegion||p.region||state.currentRegion;if(before>0)newlyUnusableId=p.id;}else if(before===0&&delta>0){p.outRegion=null;}save();}
...
function migrateOldData(){
  let changed=false;state.pokemon.forEach(p=>{if('outGym' in p){delete p.outGym;changed=true;}if(p.lives==null){p.lives=3;changed=true;}if(!p.types)p.types=[];if(!p.stats)p.stats={};if(p.image&&!p.customImage&&!p.autoImage){p.customImage=p.image;changed=true;}});if(changed)FXStorage.set(KEY,state);
}
