...
function selectedExpandedStats(stats={}){const total=STAT_ORDER.reduce((sum,k)=>sum+(Number(stats?.[k])||0),0);const items=STAT_ORDER.map(k=>{const raw=Number(stats?.[k]);const value=Number.isFinite(raw)?raw:0;const pct=Math.max(0,Math.min(100,(value/255)*100));return `<div class="selected-stat-card"><div class="selected-stat-top"><span>${statLabel[k]}</span><strong>${value||'—'}</strong></div><div class="selected-stat-track"><span style="width:${value?Math.max(3,pct):0}%"></span></div></div>`;}).join('');return `<div class="selected-stats-expand"><div class="selected-stats-expand-head"><strong>STATS BASE</strong><span>Total ${total}</span></div><div class="selected-stats-grid">${items}</div></div>`;}
...
function renderStatsModal(){const p=state.pokemon.find(x=>x.id===statsOpenId),body=$('stats-modal-body'),nav=$('stats-modal-nav');if(!p||!body)return;const dex=p.dexId?`#${String(p.dexId).padStart(4,'0')}`:'';body.innerHTML=`<div class="stats-hero"><div class="stats-hero-visual">${pokemonImg(p)}</div><div class="stats-identity"><div class="stats-kicker">${esc(p.region)}${dex?` · ${dex}`:''}</div><h2>${esc(p.nickname||p.species)}</h2>${p.nickname?`<div class="stats-species-name">${esc(p.species)}</div>`:''}<div class="type-row stats-types">${typePills(p.types)}</div><div class="stats-life-summary"><span>${hearts(p.lives,maxLives(p.region))}</span><strong>${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)} vidas`}</strong></div></div></div><div class="base-stats-panel"><div class="base-stats-title"><strong>STATS BASE</strong><span>Total ${bst(p)}</span></div><div class="base-stats-list">${statBars(p.stats)}</div></div>`;const mons=statsNavMons(),pos=mons.findIndex(x=>x.id===p.id);if(statsContext==='team'&&pos>=0&&mons.length>1){nav.hidden=false;$('stats-position').textContent=`${pos+1} / ${mons.length}`;}else nav.hidden=true;}
...
function persistFromModal(){
  ...
  if(editingId){const target=state.pokemon.find(p=>p.id===editingId);if(target){const beforeLives=Number(target.lives),beforeTeam=!!target.inTeam;Object.assign(target,data);if(data.inTeam&&!Number.isFinite(Number(target.teamOrder)))target.teamOrder=nextTeamOrder(data.region);if(beforeLives!==Number(data.lives))logAction(`${target.nickname||target.species}: ${beforeLives} → ${data.lives} vidas (edición)`,'life',data.region);if(beforeTeam!==!!data.inTeam)logAction(`${target.nickname||target.species} ${data.inTeam?'entró al':'salió del'} equipo`,'team',data.region);}}
  else{const created={id:uid(),...data};if(created.inTeam)created.teamOrder=nextTeamOrder(created.region);state.pokemon.push(created);logAction(`${created.nickname||created.species} añadido a ${created.region}`,'add',created.region);}
