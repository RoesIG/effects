...
function renderBattlePanel(){
  ...
  rivalBox.querySelectorAll('[data-battle-remove]').forEach(btn=>btn.onclick=()=>{battleOpponent.splice(Number(btn.dataset.battleRemove),1);renderBattlePanel();});
...
function renderStatsModal(){
  ...
  const view=statsDisplayMon(p),dex=p.dexId?`#${String(p.dexId).padStart(4,'0')}`:'';
  const viewSpecies=view.isMegaView?view.species:p.species;
  body.innerHTML=`<div class="stats-hero"><div class="stats-hero-visual">${pokemonImg(view)}</div><div class="stats-identity"><div class="stats-kicker">${esc(p.region)}${dex?` · ${dex}`:''}${view.isMegaView?' · FORMA MEGA':''}</div><h2>${esc(p.nickname||viewSpecies)}</h2>${p.nickname?`<div class="stats-species-name">${esc(viewSpecies)}</div>`:''}<div class="type-row stats-types">${typePills(view.types)}</div><div class="stats-ability-row"><span>HABILIDAD</span><strong>${view.ability?esc(titleCase(view.ability)):'Sin elegir'}</strong>${view.ability&&ABILITY_TYPE_MULTIPLIER[slugify(view.ability)]?'<em>Afecta eficacias</em>':''}</div><div class="stats-tools-row"><div class="mega-switch" data-mega-switch hidden></div><button class="btn small" type="button" data-compare-pokemon="${esc(p.id)}">Comparar</button></div><div class="stats-life-summary"><span>${hearts(p.lives,maxLives(p.region))}</span><strong>${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)} vidas`}</strong></div>${speciesHistoryHtml(p)}${view.isMegaView?'':`<div class="stats-evolution-action"><div><strong>EVOLUCIÓN</strong><span>Mantiene apodo, vidas, región, equipo e historial.</span></div><button class="btn small primary" type="button" data-evolve-pokemon="${esc(p.id)}">Evolucionar</button></div>`}</div></div><div class="base-stats-panel"><div class="base-stats-title"><strong>STATS BASE${view.isMegaView?' · MEGA':''}</strong><span>Total ${bst(view)}</span></div><div class="base-stats-list">${statBars(view.stats)}</div><div class="stats-efficacy-wrap"><div class="base-stats-title efficacy-title"><strong>EFICACIAS</strong><span>${(view.types||[]).map(titleCase).join(' / ')}</span></div>${typeEfficacyPanel(view)}</div></div>`;
  body.querySelector('[data-evolve-pokemon]')?.addEventListener('click',()=>openEvolution(p.id));
  body.querySelector('[data-compare-pokemon]')?.addEventListener('click',()=>openCompare(p.id));
  hydrateMegaSwitch(p);
...
async function openStats(id,context='team'){const p=state.pokemon.find(x=>x.id===id);if(!p)return;statsOpenId=id;statsContext=context;statsVariant=null;const modal=$('modal-stats');renderStatsModal();if(typeof modal.showModal==='function'){if(!modal.open)modal.showModal();}else modal.classList.add('open');if(needsDexEnrichment(p)){await ensurePokemonStats(p);renderStatsModal();renderList();renderTeamAnalysis();}}
function closeStats(){const modal=$('modal-stats');if(typeof modal.close==='function'&&modal.open)modal.close();else modal.classList.remove('open');statsOpenId=null;statsVariant=null;}
