...
const hearts=(n,max=3)=>Array.from({length:max},(_,i)=>`<span class="heart ${i<Number(n)?'':'off'}">♥</span>`).join('');
...
function renderTeam(){
  ...
  mons.forEach(p=>{
    ...
    card.innerHTML=`<div class="poke-card-head"><div><div class="poke-name">${esc(p.nickname||p.species)}</div><div class="poke-species">${esc(p.species)}${p.level?` · Nv. ${esc(p.level)}`:''}</div></div><button class="more-btn" data-edit="${p.id}" title="Editar">•••</button></div><div class="poke-visual">${pokemonImg(p)}</div><div class="type-row compact">${typePills(p.types)}</div><div class="lives-row"><div class="lives">${hearts(p.lives,maxLives(p.region))}</div><span class="life-label ${Number(p.lives)===0?'out':''}">${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)}`}</span></div><div class="life-controls"><button class="life-btn danger" data-life="-1" data-id="${p.id}" ${Number(p.lives)===0?'disabled':''}>−</button><span>VIDA</span><button class="life-btn" data-life="1" data-id="${p.id}" ${Number(p.lives)>=maxLives(p.region)?'disabled':''}>+</button></div>`;
...
function changeLife(id,delta){const p=state.pokemon.find(x=>x.id===id);if(!p)return;const before=Number(p.lives);p.lives=Math.max(0,Math.min(maxLives(p.region),before+delta));if(delta<0&&p.lives<before){p.lifeHistory=Array.isArray(p.lifeHistory)?p.lifeHistory:[];p.lifeHistory.push({region:p.region||state.currentRegion,remaining:p.lives,at:new Date().toISOString()});}if(p.lives===0){p.inTeam=false;p.outRegion=p.outRegion||p.region||state.currentRegion;if(before>0)newlyUnusableId=p.id;}else if(before===0&&delta>0){p.outRegion=null;}save();}
