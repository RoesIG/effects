...
function renderStatsModal(){
  ...
  const progression=temporary?'':`<div class="stats-life-summary"><span>${hearts(p.lives,maxLives(p.region))}</span><strong>${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)} vidas`}</strong></div>${speciesHistoryHtml(p)}${view.isMegaView?'':`<div class="stats-evolution-action"><div><strong>EVOLUCIÓN</strong><span>Mantiene apodo, vidas, región, equipo e historial.</span></div><button class="btn small primary" type="button" data-evolve-pokemon="${esc(p.id)}">Evolucionar</button></div>`}`;
  ...
  if(!temporary){body.querySelector('[data-evolve-pokemon]')?.addEventListener('click',()=>openEvolution(p.id));body.querySelector('[data-compare-pokemon]')?.addEventListener('click',()=>openCompare(p.id));}
  hydrateMegaSwitch(p);
