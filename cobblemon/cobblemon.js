...
function renderProgression(){
  ...
  const livesMax=maxLives(r),livesText=livesMax===1?'1 vida máxima por Pokémon':`${livesMax} vidas máximas por Pokémon`;
...
function renderStatsModal(){
  ...
  const progression=temporary?'':`<div class="stats-life-summary"><span>${hearts(p.lives,maxLives(p.region))}</span><strong>${Number(p.lives)===0?'INUTILIZABLE':`${p.lives}/${maxLives(p.region)} vidas`}</strong></div>${speciesHistoryHtml(p)}${view.isMegaView?'':'<div data-evolution-summary></div>'}`;
...
function renderSettings(){const r=$('settings-region')?.value||state.currentRegion,livesInput=$('region-max-lives'),hint=$('region-lives-hint');if(livesInput){livesInput.value=String(maxLives(r));livesInput.disabled=r==='Sinnoh';}if(hint)hint.textContent=r==='Sinnoh'?'Sinnoh está fijada a 1 vida: un KO de progresión deja al Pokémon inutilizable.':'El máximo se aplica a esa región completa. Al cambiarlo, ningún Pokémon conservará más vidas que el nuevo máximo.';renderUsed();}
...
function wireEvents(){
  ...
  $('settings-region').onchange=renderSettings;$('region-max-lives').onchange=e=>{const r=$('settings-region').value;if(r==='Sinnoh'){e.target.value='1';renderSettings();return;}const max=Math.max(1,Math.min(9,Number(e.target.value)||3)),before=maxLives(r);state.regionSettings[r]={...(state.regionSettings[r]||{}),maxLives:max};state.pokemon.filter(p=>p.region===r).forEach(p=>p.lives=Math.min(Number(p.lives),max));if(before!==max)logAction(`${r}: máximo de vidas ${before} → ${max}`,'settings',r);persist();renderSettings();};
