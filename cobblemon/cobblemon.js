...
function renderUnusable(){
  ...
  list.innerHTML=mons.map(p=>`<div class="unusable-card"><div class="unusable-visual">${pokemonImg(p)}</div><div class="unusable-info"><b>${esc(p.nickname||p.species)}</b><span>${esc(p.species)} · ${esc(p.region)}</span><small>♡♡♡ · ${p.outGym?`INUTILIZABLE EN GYM ${p.outGym}`:'INUTILIZABLE'}</small></div></div>`).join('');
}
...
function changeLife(id,delta){const p=state.pokemon.find(x=>x.id===id);if(!p)return;const before=Number(p.lives);p.lives=Math.max(0,Math.min(3,before+delta));if(delta<0&&p.lives<before){p.lifeHistory=Array.isArray(p.lifeHistory)?p.lifeHistory:[];p.lifeHistory.push({gym:state.currentGym,region:state.currentRegion,remaining:p.lives,at:new Date().toISOString()});}if(p.lives===0){p.inTeam=false;p.outGym=p.outGym||state.currentGym;}else if(before===0&&delta>0){p.outGym=null;}save();}
