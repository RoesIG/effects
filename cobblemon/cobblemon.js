...
function statsInline(stats={}){const ordered=['hp','attack','defense','special-attack','special-defense','speed'];return ordered.filter(k=>stats[k]!=null).map(k=>`<span><b>${statLabel[k]}</b> ${stats[k]}</span>`).join('');}
...
function render(){
  $('current-region').value=state.currentRegion;$('gym-number').value=state.currentGym;renderTeam();renderList();renderRegions();renderPreview();
}
function toggleTeam(id){const p=state.pokemon.find(x=>x.id===id);if(!p)return;if(!p.inTeam&&state.pokemon.filter(x=>x.region===p.region&&x.inTeam).length>=6){alert('El equipo de esa región ya tiene 6 Pokémon.');return;}p.inTeam=!p.inTeam;save();}
function changeLife(id,delta){const p=state.pokemon.find(x=>x.id===id);if(!p)return;p.lives=Math.max(0,Math.min(3,Number(p.lives)+delta));save();}
