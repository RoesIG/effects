...
function typeStrongAgainst(type){return TYPE_ORDER.filter(target=>Number(TYPE_EFFECT?.[type]?.[target]??1)===2);}
function typeWeakAgainst(type){return TYPE_ORDER.filter(attacker=>Number(TYPE_EFFECT?.[attacker]?.[type]??1)===2);}
function quickTypeChip(type){const color=TYPE_BADGE_COLOR[type]||'#344553';return `<span class="quick-type-chip" style="--type-color:${color}">${esc(titleCase(type))}</span>`;}
function renderTypeChart(){
  const buttons=$('type-chart-buttons'),result=$('type-chart-result');if(!buttons||!result)return;
  buttons.innerHTML=TYPE_ORDER.map(type=>`<button type="button" class="type-chart-btn ${type===selectedTypeChart?'active':''}" data-type-chart="${type}" style="--type-color:${TYPE_BADGE_COLOR[type]||'#344553'}">${esc(titleCase(type))}</button>`).join('');
  buttons.querySelectorAll('[data-type-chart]').forEach(btn=>btn.onclick=()=>{selectedTypeChart=btn.dataset.typeChart;renderTypeChart();});
  const strong=typeStrongAgainst(selectedTypeChart),weak=typeWeakAgainst(selectedTypeChart);
  result.innerHTML=`<div class="type-chart-selected"><span style="--type-color:${TYPE_BADGE_COLOR[selectedTypeChart]||'#344553'}"></span><strong>${esc(titleCase(selectedTypeChart))}</strong></div><div class="type-chart-groups"><div class="type-chart-group strong"><div class="type-chart-group-head"><strong>FUERTE CONTRA</strong><span>${strong.length}</span></div><div class="quick-type-list">${strong.length?strong.map(quickTypeChip).join(''):'<span class="type-chart-none">No golpea ×2 a ningún tipo.</span>'}</div></div><div class="type-chart-group weak"><div class="type-chart-group-head"><strong>DÉBIL CONTRA</strong><span>${weak.length}</span></div><div class="quick-type-list">${weak.length?weak.map(quickTypeChip).join(''):'<span class="type-chart-none">No tiene debilidades de tipo.</span>'}</div></div></div>`;
}
